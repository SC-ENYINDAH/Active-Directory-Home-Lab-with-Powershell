...
<#
.SYNOPSIS
    AD Security Console - Audit Function Library.
.DESCRIPTION
    Read-only Active Directory security and hygiene checks used by the
    AD Security Console dashboard. Every function in this module only
    queries AD via the ActiveDirectory module and returns structured
    finding objects. Nothing in this module modifies Active Directory.
#>
...
function Set-AdConnection {
    <#
    .SYNOPSIS
        Configures how this module talks to Active Directory.
    .DESCRIPTION
        Sets the target server/domain and optional credential used by every
        audit query in this module. Call this once at startup before running
        any check. Calling it with no arguments resets to the default
        (current user, DNS-based DC discovery).
    .PARAMETER Server
        A specific domain controller (hostname or hostname:port), or a
        domain FQDN to let the DC locator pick a controller. Optional.
    .PARAMETER Credential
        Alternate credentials to query as, e.g. when the management host is
        not domain-joined to the target. Optional.
    #>
    [CmdletBinding()]
    param(
        [string]$Server,
...
function Get-AdConnection {
    <#
    .SYNOPSIS
        Returns the current connection splat (for callers outside this module).
    #>
    [CmdletBinding()] param()
...
function Test-AdConnectivity {
    <#
    .SYNOPSIS
        Verifies the management host can reach and authenticate to the
        target domain before any audit queries are run.
    .DESCRIPTION
        Performs two checks: (1) if a specific -Server was configured, a
        TCP reachability test against the Active Directory Web Services
        port (9389), which is what the ActiveDirectory module actually
        talks to, even though it "looks like" LDAP; (2) a live
        Get-ADDomain call, which proves both connectivity and that the
        supplied (or current) credential can authenticate.
    #>
    [CmdletBinding()] param()
    ...
    if ($script:AdSplat.ContainsKey('Server')) {
        $targetHost = ($script:AdSplat['Server'] -split ':')[0]
        $result.PortTestDone = $true
        try {
            $portTest = Test-NetConnection -ComputerName $targetHost -Port 9389 -WarningAction SilentlyContinue -ErrorAction Stop
            $result.PortOpen = $portTest.TcpTestSucceeded
        } catch {
            $result.PortOpen = $false
        }
        if (-not $result.PortOpen) {
            $result.Message = "Could not reach $targetHost on TCP/9389 (Active Directory Web Services). " +
                "Check DNS resolution, routing, and that ADWS is running on the target DC, " +
                "and that no firewall between this host and the DC blocks 9389."
            return $result
        }
    }

    try {
        $domain = Get-ADDomain @script:AdSplat -ErrorAction Stop
        $result.Success     = $true
        $result.DomainName  = $domain.DNSRoot
        $result.PDCEmulator = $domain.PDCEmulator
        $result.Message     = 'Connected and authenticated successfully.'
    } catch {
        $result.Message = "Reached the network layer but the directory query failed: $($_.Exception.Message) " +
            "If this management host is not domain-joined, supply -Credential in 'user@domain.com' format " +
            "via the connection menu, and confirm the host's clock is in sync with the domain (Kerberos is " +
            "time-sensitive)."
    }
    return $result
}
...
function Get-AdStaleUserAccounts {
    ...
    $users = Get-ADUser -Filter { Enabled -eq $true } -Properties LastLogonDate -ErrorAction Stop
...
function Get-AdStaleComputerAccounts {
    ...
    $computers = Get-ADComputer -Filter { Enabled -eq $true } -Properties LastLogonDate, OperatingSystem -ErrorAction Stop
...
function Get-AdNonExpiringPasswordAccounts {
    ...
    $accounts = Search-ADAccount -PasswordNeverExpires -UsersOnly | Where-Object { $_.Enabled }

    foreach ($a in $accounts) {
        $isPrivileged = $false
        try {
            $adminCount = (Get-ADUser -Identity $a.SamAccountName -Properties AdminCount -ErrorAction Stop).AdminCount
...
function Get-AdKerberoastableAccounts {
    ...
    $users = Get-ADUser -Filter { ServicePrincipalName -like '*' } `
        -Properties ServicePrincipalName, Enabled, PasswordLastSet, AdminCount -ErrorAction Stop
...
function Get-AdAsRepRoastableAccounts {
    ...
    $users = Get-ADUser -Filter * -Properties DoesNotRequirePreAuth, Enabled, AdminCount -ErrorAction Stop |
...
function Get-AdUnconstrainedDelegation {
    ...
    $computers = Get-ADComputer -Filter { TrustedForDelegation -eq $true } -Properties TrustedForDelegation, OperatingSystem -ErrorAction Stop
    foreach ($c in $computers) {
    ...
    $users = Get-ADUser -Filter { TrustedForDelegation -eq $true } -Properties TrustedForDelegation -ErrorAction Stop
    foreach ($u in $users) {
        $findings += New-Finding -Category 'Unconstrained Delegation' -Severity 'Critical' `
...
function Get-AdAdminCountAnomalies {
    ...
    $flagged = Get-ADUser -Filter { AdminCount -eq 1 } -Properties AdminCount, Enabled -ErrorAction Stop
...
function Get-AdRecentlyCreatedAccounts {
    ...
    $users = Get-ADUser -Filter { whenCreated -ge $cutoff } -Properties whenCreated, Enabled -ErrorAction Stop
    foreach ($u in $users) {
...
function Get-AdExpiringAccounts {
    ...
    $users = Get-ADUser -Filter { Enabled -eq $true } -Properties AccountExpirationDate -ErrorAction Stop |
...
<body>
  ...
  <div class='meta'>Generated $generated on $hostName</div>
  <div>$summaryHtml</div>
  <table>
